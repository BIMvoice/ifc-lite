...
export function AppearanceSourceFields(props: Pick<AppearancePanelViewProps,
  'sources' | 'sourceId' | 'onSourceChange' | 'onRemoveSource' | 'onUpload' | 'sourceBusy' | 'sourceHelp'> & { disabled: boolean }) {
  ...
  return <section className="space-y-2" aria-labelledby={`${id}-heading`}>
    <h3 id={`${id}-heading`} className="text-xs font-medium">Source image</h3>
    <div className={`rounded-lg border border-dashed p-3 transition-colors ${dragging ? 'border-primary bg-primary/5' : 'border-border bg-muted/20'}`}
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-xs font-medium" title={source?.name}>{source?.name ?? 'Add an image to begin'}</p>
          <p className="mt-0.5 text-[11px] text-muted-foreground">{source ? `${source.width.toLocaleString()} × ${source.height.toLocaleString()} px` : 'Drop a PNG or JPEG here'}</p>
          <Button type="button" variant="ghost" size="sm" className="mt-1 h-7 px-0 text-primary" disabled={disabled} onClick={() => picker.current?.click()}>
            <Upload aria-hidden="true" />{props.sourceBusy ? 'Reading image…' : source ? 'Upload another image' : 'Choose image'}
          </Button>
        </div>
        {source && props.onRemoveSource && <Button type="button" variant="ghost" size="icon-xs" disabled={disabled}
          aria-label="Remove source image" title="Remove source image" onClick={() => props.onRemoveSource?.(source.id)}><Trash2 aria-hidden="true" /></Button>}
      </div>
      <input ref={picker} id={`${id}-file`} type="file" accept="image/png,image/jpeg,.png,.jpg,.jpeg" className="sr-only" tabIndex={-1}
        aria-label="Upload appearance image" disabled={disabled} onChange={event => {
    ...
    {props.sources.length > 0 && (props.sources.length > 1 || !source) && <label className="block space-y-1 text-[11px] text-muted-foreground">
      <span>Reuse an image</span>
      <select aria-label="Reuse an image" className={appearanceSelectClass} value={source?.id ?? ''} disabled={disabled} onChange={event => props.onSourceChange(event.target.value)}>
        {!source && <option value="" disabled>Choose an image</option>}
        {props.sources.map(source => <option key={source.id} value={source.id}>{source.name}</option>)}
      </select>
    </label>}
    <p className="text-[10px] leading-relaxed text-muted-foreground">{props.sourceHelp ?? `PNG or JPEG · up to ${limits.maxImageBytes / 1048576} MB and ${(limits.maxPixels / 1e6).toFixed(1)} megapixels · maximum edge ${limits.maxDimension.toLocaleString()} px`}</p>
  </section>;
}
